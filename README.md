...
### 박진영 — 베이스 모델 성능 및 Feature Importance 분석
...
|------|------------|
| CatBoost | 0.7404 |
| FT-Transformer | 0.7395 |
| ResNet-MLP | 0.7381 |
| **3-way Ensemble (최종)** | **0.7409** |

> 최적 앙상블 가중치: Cat=0.52 · FT=0.31 · MLP=0.17

---

#### ROC-AUC 곡선

![박진영 ROC-AUC 곡선](images/roc_auc_jinyoung.png)

CatBoost(0.7404)가 단일 모델 중 가장 높은 AUC를 기록했으며, FT-Transformer(0.7395)와 ResNet-MLP(0.7381)가 뒤를 이었습니다. 세 모델의 오류 패턴 차이를 활용한 3-way 앙상블이 AUC 0.7409로 개별 모델을 모두 상회했습니다.

---

#### CatBoost Feature Importance

![박진영 CatBoost Feature Importance](images/feature_importance_cat_jinyoung.png)

**CatBoost**에서는 피처 엔지니어링으로 생성한 `파이프_효율`(15.31)과 `이식_비율`(14.84)이 1~2위를 차지했습니다. 원본 변수인 `이식된_배아_수`(9.39)가 3위에 올랐으며, 이는 파생 변수가 단순 원본 수치보다 더 강한 신호를 제공함을 보여줍니다. `배아 이식 경과일_missing`(3.27), `혼합_이식_간격`(3.25), `실효_난자_나이`(3.23) 등 시간 간격 및 도메인 파생 변수도 상위권을 형성했습니다.

---

#### 3-way 앙상블 Feature Importance (Spearman ρ)

![박진영 앙상블 Feature Importance](images/feature_importance_ensemble_jinyoung.png)

3-way 앙상블에서는 `배양_기간`, `배아 이식 경과일`, `혼합_이식_간격`, `해동_이식_간격`이 Spearman ρ 0.722로 공동 1위를 기록했습니다. 시술 단계별 시간 간격 변수들이 앙상블 예측과 가장 높은 상관관계를 보이며 핵심 피처로 부상했습니다. `이식_포배기`(0.642)와 `배아 이식 경과일_missing`(0.622)이 뒤를 이어, 배아 이식 시기의 중요성이 앙상블에서도 확인됩니다.

---

#### CatBoost vs 3-way 앙상블 — Feature Importance 비교

| 피처 | CatBoost 순위 | 앙상블 순위 |
|------|------------|-----------|
| 파이프_효율 | 🥇 1위 (15.31) | — |
| 이식_비율 | 🥈 2위 (14.84) | — |
| 이식된_배아_수 | 🥉 3위 (9.39) | 21위 (0.367) |
| 배양_기간 | 13위 (2.58) | 🥇 1위 (0.722) |
| 배아 이식 경과일 | 10위 (2.83) | 🥇 1위 (0.722) |
| 혼합_이식_간격 | 7위 (3.25) | 🥇 1위 (0.722) |
| 이식_포배기 | — | 5위 (0.642) |
| 실효_난자_나이 | 8위 (3.23) | 9위 (0.442) |

CatBoost는 효율성 파생변수(`파이프_효율`, `이식_비율`)를 가장 중요하게 평가한 반면, 앙상블 관점에서는 시간 간격 변수(`배양_기간`, `혼합_이식_간격`)가 예측력의 핵심으로 부상했습니다. 이는 각 모델이 서로 다른 피처 공간을 강점으로 삼고 있음을 보여주며, 이종 앙상블의 다양성 효과를 뒷받침합니다.
...
#### 피처 엔지니어링이 성능을 결정했습니다.

피처 엔지니어링 방식에 따라 모델 성능과 Feature Importance 패턴이 크게 달라졌습니다.
특히 `파이프_효율`, `이식_비율` 등 파생 변수가 CatBoost에서 원본 변수보다
높은 importance를 기록하며, 도메인 지식 기반 피처 엔지니어링이 단순 원본 변수보다
더 강한 신호를 제공함을 확인했습니다.

| 인사이트 | 근거 |
|---------|------|
| 도메인 플래그(`배반포기_이식`)가 딥러닝에서 압도적 1위를 차지합니다. | FTT·SAINT 모두 0.5~0.6을 기록했습니다. |
| 파생 변수가 원본 변수를 압도합니다. | `파이프_효율`(15.31)이 `이식된_배아_수`(9.39)보다 높게 나타났습니다. |
| 시간 간격 변수가 앙상블 예측력의 핵심으로 작용합니다. | `배양_기간`과 `혼합_이식_간격`의 Spearman ρ가 0.722를 기록했습니다. |

---

#### 트리 모델과 딥러닝 모델은 서로 다른 피처 공간을 학습합니다.

트리 모델은 **연속형 수치**(`이식된_배아_수`, `배아_이식_경과일`)에 강하게 반응한 반면,
딥러닝 모델은 **도메인 플래그**(`배반포기_이식`)와 **상호작용 피처**를 더 중요하게 학습했습니다.
이러한 차이가 이종 앙상블에서 성능을 향상시키는 핵심 근거가 됩니다.

| 모델 유형 | 강점 피처 | 대표 예시 |
|---------|---------|---------|
| 트리 (LightGBM·XGBoost·CatBoost) | 연속형 수치 데이터 | `이식된_배아_수`, `배아_이식_경과일` |
| 딥러닝 (FTT·SAINT·ResNet-MLP) | 도메인 플래그 및 상호작용 데이터 | `배반포기_이식`, `연령_그룹` |

---

#### 앙상블 전략보다 결측치 처리 방식이 더 큰 변수로 작용했습니다.

MICE 보간과 0 대체라는 결측치 처리 방식의 차이가 Feature Importance 패턴에
뚜렷한 차이를 만들었습니다. 배아 및 난자 변수 간의 강한 상관관계(0.87~0.94)를
활용한 MICE 보간법이 해당 변수군의 예측력을 더욱 정교하게 살려냈습니다.

---

### 2. 도메인 인사이트

#### 의학적 근거가 모델에서 그대로 재현되었습니다.

EDA 단계에서 도메인 지식을 바탕으로 설정한 가설들이 모든 모델에서 일관되게 확인되었습니다.

| 도메인 가설 | 모델 확인 결과 |
|-----------|-------------|
| 5일차(배반포기) 이식이 성공률을 1.9배 높입니다. | `배반포기_이식` 피처가 딥러닝 전 모델에서 1위를 기록했습니다. |
| 35세 이후부터 성공률이 급락합니다. | `연령_그룹` 피처가 전 모델에서 상위 2~3위를 차지했습니다. |
| 배아 수가 많을수록 성공률이 향상됩니다. | `이식된_배아_수` 피처가 트리 모델에서 1위를 기록했습니다. |
| 반복해서 실패할수록 착상률이 감소합니다. | 이력 기반 파생 변수들이 상위권에 진입했습니다. |

---

#### 환자의 나이보다 난자의 나이가 더 결정적인 역할을 합니다.

44세 이상 환자가 기증 난자를 사용할 경우 성공률이 27.5%로, 본인 난자를 사용했을 때(3.5%) 대비 약 8배에 달합니다.
`실효_난자_나이` 파생 변수가 모델에서 일관되게 상위권을 차지했으며, 이를 통해
**시술 시 환자의 물리적 나이보다 실제 사용되는 난자의 나이가 임신 성공에 더 결정적인 요인**임을
데이터로 재확인했습니다.

---

#### 시술 단계별 시간 관리가 성공률을 좌우합니다.

`배양_기간`, `혼합_이식_간격`, `해동_이식_간격` 등 시술 프로세스상의 시간 간격 변수들이
최종 앙상블 모델에서 Spearman ρ 0.722로 공동 1위를 기록했습니다.
이는 단순히 어떤 시술을 진행하느냐보다, **각 단계 간의 시간 관리가 배아 품질과 착상 성공에
직접적인 영향을 미친다**는 의학적 사실을 데이터가 완벽하게 뒷받침해 주는 결과입니다.